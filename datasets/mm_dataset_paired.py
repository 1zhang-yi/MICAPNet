...
class mmdataset(Dataset):
    def __init__(self, discretizer, normalizer, listfile, ehr_dir, image_dir, transforms, return_names=True, period_length=48.0, ehr_pkl_fpath=None):
        ...
        if ehr_pkl_fpath is not None and os.path.isfile(ehr_pkl_fpath):
            ...
            for k in self.names:
                ...
                self.processed_data[k] = {'data': data, 'image': image, 'ys': ys}
    ...
    def read_by_file_name(self, index, time_bound=None):
        ...
        image = self.data_map[index]['image']
    ...
    def __getitem__(self, index, time_bound=None):
        ...
        image = ret['image']
        image_path = f'{self.image_dir}/{image}.jpg'
...
def get_mm_datasets(discretizer, normalizer, args):
    train_ds = mmdataset(discretizer, normalizer,
                         listfile=f'{args.ehr_data_dir}/{args.task}/train/train_listfile_with_images.csv',
                         ehr_dir=os.path.join(args.ehr_data_dir, f'{args.task}/train'),
                         image_dir=f'{args.image_dir}/train_images_resized',
                         transforms=train_transform,
                         ehr_pkl_fpath=args.train_mm_pkl_fpath_paired)
    test_ds = mmdataset(discretizer, normalizer,
                        listfile=f'{args.ehr_data_dir}/{args.task}/test/test_listfile_with_images.csv',
                        ehr_dir=os.path.join(args.ehr_data_dir, f'{args.task}/test'),
                        image_dir=f'{args.image_dir}/test_images_resized',
                        transforms=test_transform,
                        ehr_pkl_fpath=args.test_mm_pkl_fpath_paired)
    return train_ds, test_ds


def get_data_loader(discretizer, normalizer, args):
    train_ds, test_ds = get_mm_datasets(discretizer, normalizer, args)
...
def my_collate(batch):
    ...
    return [x, image, targets, seq_length]
